...
/*	Variations	*/
	...
	function ajax_add_new_single_variation() {
		...
		foreach ( unserialize(WPSHOP_ATTRIBUTE_PRICES) as $price_attribute_code) {
			...
			if ( !empty($price_attr_def) && !empty($variation_specific_definition[$price_attr_def->data_type]) && is_array($variation_specific_definition[$price_attr_def->data_type]) && !array_key_exists($price_attribute_code, $variation_specific_definition[$price_attr_def->data_type]) ) {
